...
size_t es_token_t_read(es_token_t *token, void *p) {
	...
	if (data_has) {
	    ...
	    memcpy(token->data,p+size,sizeof(uint8_t) * token->size);
	    size += token->size * sizeof(uint8_t);
	}
//structEnd()
    return size;
...
size_t es_string_token_t_read(es_string_token_t *string_token, void *p) {
	...
	if (data_has) {
	    ...
	    memcpy(string_token->data,p+size,sizeof(char) * string_token->length);
	    size += string_token->length * sizeof(char);
	}
//structEnd()
    return size;
...
size_t es_muted_path_t_read(es_muted_path_t *muted_path, void *p) {
	...
	if (events_has) {
	    ...
	    memcpy(muted_path->events,p+size,sizeof(es_event_type_t) * muted_path->event_count);
	    size += muted_path->event_count * sizeof(es_event_type_t);
	}
	//onCustomStruct(_:_:_:)
	...
	*(size_t*)(p+size) = muted_paths->count;
	...
	if (muted_paths->paths) {
	    *(_Bool *)(p+size) = true;
	    size += sizeof(_Bool);
	    for (int i = 0; i < muted_paths->count; i++) {
	        size += es_muted_path_t_write((es_muted_path_t*)&(muted_paths->paths[i]), p+size);
	    }
	} else {
	    *(_Bool *)(p+size) = false;
	    size += sizeof(_Bool);
...
size_t es_muted_paths_t_read(es_muted_paths_t *muted_paths, void *p) {
	...
	if (paths_has) {
	    ...
	    for (int i = 0; i < muted_paths->count; i++) {
	        size += es_muted_path_t_read((es_muted_path_t*)&(muted_paths->paths[i]), p+size);
	    }
	}
//structEnd()
    return size;
...
size_t es_muted_process_t_read(es_muted_process_t *muted_process, void *p) {
	...
	if (events_has) {
	    ...
	    memcpy(muted_process->events,p+size,sizeof(es_event_type_t) * muted_process->event_count);
	    size += muted_process->event_count * sizeof(es_event_type_t);
	}
//structEnd()
    return size;
	...
	*(size_t*)(p+size) = muted_processes->count;
	...
	if (muted_processes->processes) {
	    *(_Bool *)(p+size) = true;
	    size += sizeof(_Bool);
	    for (int i = 0; i < muted_processes->count; i++) {
	        size += es_muted_process_t_write((es_muted_process_t*)&(muted_processes->processes[i]), p+size);
	    }
	} else {
	    *(_Bool *)(p+size) = false;
	    size += sizeof(_Bool);
...
size_t es_muted_processes_t_read(es_muted_processes_t *muted_processes, void *p) {
	...
	if (processes_has) {
	    ...
	    for (int i = 0; i < muted_processes->count; i++) {
	        size += es_muted_process_t_read((es_muted_process_t*)&(muted_processes->processes[i]), p+size);
	    }
	}
//structEnd()
    return size;
